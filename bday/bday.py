...
class Bday(commands.Cog):
    ...
    async def birthday(self, ctx, user: discord.Member):
        ...
        if not await self.config.guild(ctx.guild).bdayRole() == 0:
            await user.add_roles(ctx.guild.get_role(await self.config.guild(ctx.guild).bdayRole()), reason="It's their birthday!") 
            await ctx.send("Happy birthday {} !".format(user.mention()))
        else:
            await ctx.send("You need to configure a birthday role first by using ``[p]setbirthday``.")
    ...
    async def clearbirthdays(self, ctx):
        ...
        for user in ctx.guild.get_role(await self.config.guild(ctx.guild).bdayRole()).members:
            await user.remove_roles(ctx.guild.get_role(await self.config.guild(ctx.guild).bdayRole()), reason="It's a new day. The birthdays are over.")